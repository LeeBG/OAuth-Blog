...
public class PostController {
	...
	public String detail(@PathVariable int id,Model model) {
		...
		return "post/detail";					//ViewResolver발동 = jsp파일을 찾아줄것이다.
	}
	...
	public @ResponseBody CMRespDto<?> deleteById(@PathVariable int id){//왜냐하면 자바스크립트한테 파일을 응답할 수는 없기 때문에rest
		postService.삭제하기(id);
		return new CMRespDto<>(1,null);
	}
	
	@PutMapping("/post/{id}")
	public @ResponseBody CMRespDto<?> update(@PathVariable int id,@RequestBody PostSaveReqDto postSaveReqDto){//왜냐하면 자바스크립트한테 파일을 응답할 수는 없기 때문에rest
